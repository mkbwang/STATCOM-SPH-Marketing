---
title: "SPH Website User Survey Report"
output: 
...
```{r}
include_graphics("data_summary.png") 
```


### Extra Comments on Data
1. The data are provided by David Washington and Eric Sanotti from SPH Marketing Team. 
...
```{r}
include_graphics("Analysis_methods.png") 
```


### Extra Comments on Analysis
1. The analysis was done by Rachael Beer, Nicholas Hartman and Mukai Wang. STATCOM leader Steve Salerno and faculty advisor Tom Braun also provided advice. All the analysis were done in R.
2. THe likert plot was done with the [likert](https://cran.r-project.org/web/packages/likert/likert.pdf) R package.
3. [Wilcoxon tests](https://en.wikipedia.org/wiki/Wilcoxon_signed-rank_test) are used for hypotheses tests of ranking data.
4. The preprocessing includes removing numbers, removing punctuations, changing all the words to lowercase and [stemming words](https://en.wikipedia.org/wiki/Stemming). Therefore in the individual word ranking tables, some tokens are chopped versions of certain words that can be easily inferred.
5. [TF-IDF](https://en.wikipedia.org/wiki/Tf%E2%80%93idf) takes into consideration both the frequency and the "uncommoness" of tokens.
6. [STM](https://www.structuraltopicmodel.com/) identifies certain words that tend to appear together in the documents. The clusterings of words are called "topics".
...
ggplotly(elements_open_Q7[[1]])
```

Row {data-height=750}
-------------------------------------

### Rank Distribution by Student Affiliation

```{r}
ggplotly(elements_open_Q7[[2]])
```
...
```{r}
elements_class_Q12 = readRDS("results/Class_Q12.rds")
